...
export namespace Value {
  ...
  export function resolve(scope: DeclarationScope, context: TypeChecker): void {
    scope.node.argumentList.args.forEach(
      (arg) => (scope.values[arg.ident.value] = make(scope, arg.ident)),
    );

    travel(scope.node.exprs, isAssignmentExpression).forEach(
      (assignmentExpr) => {
        scope.values[assignmentExpr.left.value] = make(
          scope,
          assignmentExpr.left,
        );
        ...
        travel(assignmentExpr.right, isIdentifierExpression)
          .filter(
            (identExpr) => identExpr.ident.value === assignmentExpr.left.value,
          )
          .forEach((identExpr) => {
    ...
    travel(scope.node.exprs, isIdentifierExpression).forEach((identExpr) =>
      find(scope, identExpr.ident).mapOrElse<unknown>(
        () => {
          context.diagnostics.push({
            source: scope.file.file,
            message: `Using undeclared value name '${identExpr.ident.value}'.`,
